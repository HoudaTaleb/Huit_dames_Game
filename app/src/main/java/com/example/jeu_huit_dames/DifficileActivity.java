...
public class DifficileActivity extends AppCompatActivity {
    ...
    long threeMinutesInMillis = 3 * 60 * 1000; // 3 minutes en millisecondes
    private Jeu jeu;
    ...
    class Jeu {
        ...
        int get_A(){return a;}
        boolean isMenace(){return menace;}
        ...
        void affecter_reine(int lig, int col) {
            ...
            if (positions_reines[lig][col] == 1) {
                ...
                if (verification1()) {
                    ...
                    menace = true;
                    StringBuilder message = new StringBuilder("Vous ne pouvez pas jouer ici, une reine menace une autre.\n\n");


                    // Créer une boîte de dialogue d'alerte
                    AlertDialog.Builder builder = new AlertDialog.Builder(DifficileActivity.this, R.style.AlertDialogStylealerte);
...
// Définir la base du chronomètre à l'heure actuelle plus trois minutes en millisecondes

        simpleChronometer.setBase(currentTime + threeMinutesInMillis);

// Démarrer le chronomètre
        simpleChronometer.start();

        Button restart = findViewById(R.id.resetChronometer);


        restart.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Réinitialiser la base du chronomètre à l'heure actuelle plus trois minutes en millisecondes
                simpleChronometer.setBase(SystemClock.elapsedRealtime() + threeMinutesInMillis);
                raffrichir_jeu();
    ...
    private void interface_affecter_reine(int lig, int col) {
        ...
        dessiner_grille();
        // Calculer le nombre de cases valides restantes
        int casesValidesRestantes = countValidCases();

        // Afficher un Toast avec le nombre de cases valides restantes
        Toast.makeText(DifficileActivity.this, "Nombre de cases valides restantes : " + casesValidesRestantes, Toast.LENGTH_SHORT).show();
        ...
        // Vérifier si toutes les 8 dames ont été placées
        if (jeu.getNombreDeReines() == 8) {
            interface_agagner();
            simpleChronometer.stop();
            en_cours = 0;
        }

        if (casesValidesRestantes == 0 && jeu.getNombreDeReines() != 8) {
            AlertDialog.Builder d = new AlertDialog.Builder(this, R.style.AlertDialogStylelose);
            d.setTitle(R.string.Perdu);
    ...
    // Méthode pour compter le nombre de cases valides restantes
    private int countValidCases() {
        int count = 0;
        for (int i = 0; i < Jeu.taille_grille; i++) {
            for (int j = 0; j < Jeu.taille_grille; j++) {
                if (jeu.position_actuelle(i, j) == 0 && !isInvalidCell(i, j)) {
                    count++;
                }
            }
        }
    ...
    private boolean isInvalidCell(int row, int col) {
        // Vérifier si la case est déjà occupée par une reine
        if (jeu.position_actuelle(row, col) == 1) {
            return true; // La case est déjà occupée, donc invalide
        }

        // Vérifier s'il y a une menace sur la ligne
        for (int j = 0; j < Jeu.taille_grille; j++) {
            if (jeu.position_actuelle(row, j) == 1) {
                return true; // Une reine occupe déjà cette ligne, donc la case est invalide
            }
        }

        // Vérifier s'il y a une menace sur la colonne
        for (int i = 0; i < Jeu.taille_grille; i++) {
            if (jeu.position_actuelle(i, col) == 1) {
                return true; // Une reine occupe déjà cette colonne, donc la case est invalide
            }
        }

        // Vérifier s'il y a une menace sur la diagonale principale
        int diff = col - row;
        for (int i = 0; i < Jeu.taille_grille; i++) {
            int j = i + diff;
            if (j >= 0 && j < Jeu.taille_grille && jeu.position_actuelle(i, j) == 1) {
                return true; // Une reine occupe déjà cette diagonale principale, donc la case est invalide
            }
        }

        // Vérifier s'il y a une menace sur la diagonale secondaire
        int sum = col + row;
        for (int i = 0; i < Jeu.taille_grille; i++) {
            int j = sum - i;
            if (j >= 0 && j < Jeu.taille_grille && jeu.position_actuelle(i, j) == 1) {
                return true; // Une reine occupe déjà cette diagonale secondaire, donc la case est invalide
            }
        }

        // Si aucune menace n'est détectée, la case est valide
        return false;
    }
